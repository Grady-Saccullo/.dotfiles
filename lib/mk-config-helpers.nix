...
let
	...
	hashSystemConfig = cfg: "${cfg.platform}-${cfg.architecture}-${cfg.config-type}-${cfg.user}";

	genSystemConfig = systems: make: nixpkgs.lib.genAttrs (map (s: s.config-type) systems) (config:
		let 
