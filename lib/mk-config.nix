...
let
	...
	buildSystem = hm-modules: mkSystemModules {
	...
	built-config = if isPlatform "darwin" then
		let 
			system = buildSystem [
		...
		in inputs.darwin.lib.darwinSystem system
	else if isPlatform "nixos" then 
		let 
			system = buildSystem [
		...
		in nixpkgs.lib.nixosSystem system
	else 
		buildSystem [ home-manager-config ];
in built-config
